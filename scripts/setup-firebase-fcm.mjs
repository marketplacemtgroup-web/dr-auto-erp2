...
if (parsed.project_id !== 'oficinadobeto-portal') {
  console.warn(
    `Aviso: project_id é "${parsed.project_id}" (esperado oficinadobeto-portal). Confira se o JSON é do projeto certo.`,
  );
}
...
console.log('\n=== Firebase FCM — variáveis para a Vercel (oficinadobeto-api) ===\n');
...
console.log('  1. https://vercel.com → projeto oficinadobeto-api → Settings → Environment Variables');
console.log('  2. Adicione FIREBASE_SERVICE_ACCOUNT (Production + Preview)');
console.log('  3. Redeploy da API');
console.log('  4. Confira: https://oficinadobeto-api.vercel.app/api/env-check → push.firebaseFcm: true\n');
