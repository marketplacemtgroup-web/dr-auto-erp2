-- ERP Financeiro Profissional — migration aditiva
...
-- financial_account_movements (razão)
...
CREATE INDEX "financial_account_movements_organization_id_account_id_movement_date_idx"
...
-- Backfill: Caixa Principal por organização
...
SELECT
    ...
    o."id",
    'Caixa Principal',
    'CAIXA'::"FinancialAccountType",
    0,
    0,
    'ACTIVE'::"FinancialAccountStatus",
    '#16A34A',
    true,
    true,
    NOW(),
...
-- Backfill: centros de custo padrão
...
FROM "organizations" o
CROSS JOIN (VALUES
    ('Oficina'), ('Administração'), ('Estoque'), ('Recepção'),
...
-- Vincular sessões de caixa à conta principal
UPDATE "cash_register_sessions" crs
SET "account_id" = fa."id"
FROM "financial_accounts" fa
WHERE crs."organization_id" = fa."organization_id"
  AND fa."is_primary" = true
  AND crs."account_id" IS NULL;

-- Vincular lançamentos pagos à conta principal (legado)
UPDATE "financial_entries" fe
SET "account_id" = fa."id"
FROM "financial_accounts" fa
WHERE fe."organization_id" = fa."organization_id"
  AND fa."is_primary" = true
  AND fe."status" = 'PAID'
  AND fe."account_id" IS NULL;

-- Sincronizar amount_paid com amount_received para registros já pagos
UPDATE "financial_entries"
SET "amount_paid" = COALESCE("amount_received", "amount")
