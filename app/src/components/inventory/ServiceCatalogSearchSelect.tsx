...
export function serviceCatalogDisplayLabel(s: ServiceCatalogRow) {
...
type Props = {
  value: string;
  onChange: (catalogItemId: string, item: ServiceCatalogRow | null) => void;
  placeholder?: string;
  allowManual?: boolean;
  manualLabel?: string;
};

export default function ServiceCatalogSearchSelect({
  value,
  onChange,
  placeholder = "Digite o nome ou categoria do serviço...",
  allowManual = true,
  manualLabel = "Manual / outro (sem vincular ao catálogo)",
}: Props) {
  ...
  const { data: searchResult, isLoading: searchLoading } = useQuery({
    queryKey: ["service-catalog-search", debouncedQuery, token],
    queryFn: () => api.serviceCatalog(token!, debouncedQuery),
  ...
  const { data: selectedItem } = useQuery({
    queryKey: ["service-catalog-item", value, token],
    queryFn: () => api.serviceCatalogItem(token!, value),
    enabled: !!token && !!value,
    staleTime: QUERY_STALE_TIME_MS,
    gcTime: QUERY_GC_TIME_MS,
  });
  ...
  function selectItem(item: ServiceCatalogRow) {
      ...
      <div className="rounded-lg border border-[#0E7490]/30 bg-[#ECFEFF] px-3 py-2.5 flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-[12px] text-[#64748B] mt-0.5">
            {selectedItem.category ?? "Sem categoria"}
            {" · "}
            {formatMoney(selectedItem.defaultPrice)}
            {selectedItem.estimatedMinutes ? ` · ${selectedItem.estimatedMinutes} min` : ""}
          </p>
        </div>
        <button
          type="button"
      ...
      {open ? (
        <div className="absolute z-10 left-0 right-0 mt-1.5 rounded-lg border border-[#E2E8F0] bg-white shadow-lg overflow-hidden">
          {debouncedQuery.length === 0 ? (
            <p className="px-3 py-3 text-[12px] text-[#94A3B8]">Digite para buscar no catálogo.</p>
          ) : searchLoading ? (
